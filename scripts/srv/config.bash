...
check_config() {
    ...
    [ "$run_mode" = "docker_compose" ] && whoops "run_mode=docker_compose is not yet implemented"
...
setup_run_mode() {
    case "$network_mode" in
        prod)
            case "$arg_run_mode" in
                docker_compose|systemd) run_mode="$arg_run_mode" ;;
...
local base(o) = o + {
    ...
    db +: {
        ...
        port : $(. ./env.sh && echo $DB_PORT),
        host : "$(. ./env.sh && echo $DB_HOST)",
        'no-tls' : $(. ./env.sh && if [ $DB_NO_TLS -eq 1 ]; then echo "true"; else echo "false"; fi ),
    },
